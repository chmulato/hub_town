...
- Infra (RabbitMQ): habilitado no docker-compose com UI de gestão.
  - Guia: `doc/RABBIT_MQ_SETUP.md` (estratégia de exchange/queues, retry/DLQ, boas práticas).
...
- Operacional
  ...
  - RabbitMQ disponível via docker-compose para futuras fases de ingestão (ver `doc/RABBIT_MQ_SETUP.md`).
...
- [ ] Adicionar testes mínimos de integração (Node) para `GET /api/marketplace/:id/orders`, `GET /api/orders/search`, `GET /api/orders/stats` (avaliar cobertura atual do `tests/api-integration-test.js`). Ver visão geral: `tests/README.md`.
